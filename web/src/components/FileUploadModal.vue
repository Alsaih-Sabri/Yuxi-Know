<template>
  <a-modal v-model:open="visible" :title="$t('fileUpload.title')" width="800px" @cancel="handleCancel">
    <template #footer>
      <div class="footer-container">
        <a-button type="link" class="help-link-btn" @click="openDocLink">
          <CircleHelp :size="14" /> {{ $t('fileUpload.buttons.docHelp') }}
        </a-button>
        <div class="footer-buttons">
          <a-button key="back" @click="handleCancel">{{ $t('fileUpload.buttons.cancel') }}</a-button>
          <a-button
            key="submit"
            type="primary"
            @click="chunkData"
    ...
    <div class="add-files-content">
      ...
      <div class="settings-panel">
        ...
        <div class="setting-row two-cols">
          <div class="col-item">
            <div class="setting-label">{{ $t('fileUpload.settings.storageLocation') }}</div>
            <div class="setting-content flex-row">
              <a-tree-select
                ...
                :placeholder="$t('fileUpload.settings.selectFolder')"
                allow-clear
            ...
            <p class="param-description">{{ $t('fileUpload.settings.folderDescription') }}</p>
          </div>
          <div class="col-item">
            <div class="setting-label">
              {{ $t('fileUpload.settings.ocrEngine') }}
              <a-tooltip :title="$t('fileUpload.settings.checkStatus')">
                <ReloadOutlined
            ...
            <div class="setting-content">
              ...
              <p class="param-description">
                <template v-if="!isOcrEnabled"> {{ $t('fileUpload.ocr.disabled') }} </template>
                <template v-else-if="selectedOcrStatus === 'healthy'">
                  {{ selectedOcrMessage || $t('fileUpload.ocr.healthy') }}
                </template>
                <template v-else-if="selectedOcrStatus === 'unknown'">
                  {{ $t('fileUpload.ocr.checkStatus') }}
                </template>
                <template v-else>
                  {{ selectedOcrMessage || $t('fileUpload.ocr.unhealthy') }}
                </template>
              </p>
            </div>
          </div>
        ...
        <div class="setting-row">
          <div class="col-item">
            <div class="setting-label">
              <a-checkbox v-model:checked="autoIndex">{{ $t('fileUpload.settings.autoIndex') }}</a-checkbox>
            </div>
            <div class="setting-content" v-if="autoIndex">
              ...
              <template v-else>
                <div class="lightrag-tip">
                  ...
                  <span>{{ $t('fileUpload.lightrag.autoIndexTip') }}</span>
                </div>
              </template>
            </div>
          </div>
      ...
      <div v-if="hasPdfOrImageFiles && !isOcrEnabled" class="inline-alert warning">
        <Info :size="16" />
        <span>{{ $t('fileUpload.alerts.pdfOcrSuggestion') }}</span>
      </div>
      ...
      <div class="upload-area">
        <a-upload-dragger
          ...
          @drop="handleDrop"
        >
          <p class="ant-upload-text">{{ $t('fileUpload.uploadArea.dragText') }}</p>
          <p class="ant-upload-hint">{{ $t('fileUpload.uploadArea.supportedTypes') }} {{ uploadHint }}</p>
      ...
      <div v-if="sameNameFiles.length > 0" class="conflict-files-panel">
        <div class="panel-header">
          ...
          <span>{{ $t('fileUpload.alerts.duplicateFiles', { count: sameNameFiles.length }) }}</span>
        </div>
...
const uploadModeOptions = computed(() => [
  {
    ...
    label: h('div', { class: 'segmented-option' }, [
      ...
      h('span', { class: 'option-text' }, t('fileUpload.uploadMode.file'))
    ])
  },
  {
    value: 'folder',
    label: h('div', { class: 'segmented-option' }, [
      ...
      h('span', { class: 'option-text' }, t('fileUpload.uploadMode.folder'))
    ])
  }
])
